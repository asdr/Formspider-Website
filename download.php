...
<script type="text/javascript" >
	$(document).ready(function() {
		...
		var $dialog = $('<div id="systemRequirementsDialog"></div>')
			...
			.dialog({
				autoOpen: false,
				title: 'Formspider 1.3.1 Installation Requirements',
				width:600
			});
		...
		$( "#downloadWindows" )
			...
			.click(function() {
				if(checkForm(nameField, emailField)) {
					downloadURL("http://cdn.theformspider.com/cdn/Formspider_1.3.1.zip");
		...
		$( "#downloadLinux" )
			...
			.click(function() {
				if(checkForm(nameField, emailField)) {
					downloadURL("http://cdn.theformspider.com/cdn/Formspider_1.3.1.jar");
					logDownload(nameField.value, emailField.value);
		...
		function logDownload(name, email) {
			...
			$.ajax({
			  type: "GET",
			  url: "product.php",
			  data: "descid=Download&name="+ name +"&mail="+ email+"&version=1.3.1",
			  async: true,
